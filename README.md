# Hi there, I'm sibtain hyder shah

 **High School Student & Aspiring Aerospace Engineer**  
 Saudi Arabia | Grade 11 (American Curriculum)

---

##  About Me
-  **Focus:** Aerospace Engineering, Planetary Exploration Robotics, & Computational Physics.
-  **Goals:** Target score 1530+ on the Digital SAT (March 2027) & pursuing undergraduate Aerospace Engineering at top-tier institutions.
-  **Coursework:** Advanced Physics, Mathematics, CAD, and mandatory Saudi Ministry of Education studies (Arabic, Islamic Studies, Saudi History).

---

##  Featured Projects & Engineering Highlights

###  VEX V5 Extreme-Terrain Planetary Rover & Lift Mechanism
- **Overview:** Designed and built a VEX V5 robotics system engineered for extreme terrain navigation and sample collection.
- **Key Features:**
  - Integrated a 4-bar linkage lift mechanism for vertical payload clearance.
  - Custom gear-driven drivetrain optimized for torque and traction.
  - Fabricated scrap-built claw end-effector for versatile object acquisition.
- **Tools Used:** VEX V5 System, Tinkercad (3D Modeling & Layout)

### Physics Analysis & Electromegnetism
- **overview:** Conducted experimental analysis on fundamental physics concepts and electromagnetic phenomena.
- **Key Project:** Studied Hans Christian Oersted’s electromagnetism experiment to analyze magnetic fields created by electrical conductors and their practical engineering applications.

---

##  Tech Stack & Technical Tools

| Category | Tools & Technologies |
| :--- | :--- |
| **Engineering & CAD** | Tinkercad, VEXcode V5, VEX V5 Hardware Systems|
| **Programming & Tools** | Git, GitHub, Markdown |
| **Data & Analysis** | Scientific Calculator (Statistical & Regression Analysis), Excel |
| **Academic Platforms** | Toddle, Digital SAT Prep |

---

##  Connect With Me
-  **Email:** sibainhyershah@gmail.com

---
> *“Failure is an option here. If things are not failing, you are not innovating enough.” — Elon Musk*
